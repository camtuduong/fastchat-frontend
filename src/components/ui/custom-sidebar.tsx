...
const CustomSidebarContext = React.createContext<SidebarContextProps | null>(
  null,
);
...
function useCustomSidebar() {
  ...
  if (!context) {
    throw new Error(
      "useCustomSidebar must be used within a CustomSidebarProvider.",
    );
  }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:-left-[calc(var(--sidebar-width)+(--spacing(2)))] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:-right-[calc(var(--sidebar-width)+(--spacing(2)))] md:flex",
          // Adjust the padding for floating and inset variants.
          variant === "floating" || variant === "inset"
            ? "mx-2 py-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)]"
...
function CustomSidebarSeparator({
  className,
  ...props
...
function CustomSidebarGroupLabel({
  className,
  asChild = false,
  ...props
...
function CustomSidebarGroupContent({
  className,
  ...props
}: React.ComponentProps<"div">) {
  return (
...
const customSidebarMenuButtonVariants = cva(
...
function CustomSidebarMenuButton({
...
} & VariantProps<typeof customSidebarMenuButtonVariants>) {
  const Comp = asChild ? Slot.Root : "button";
  ...
  const button = (
    <Comp
      ...
      className={cn(
        customSidebarMenuButtonVariants({ variant, size }),
        className,
      )}
      {...props}
    />
...
function CustomSidebarMenuAction({
  className,
  asChild = false,
  showOnHover = false,
  ...props
...
function CustomSidebarMenuBadge({
  className,
  ...props
}: React.ComponentProps<"div">) {
  return (
...
function CustomSidebarMenuSkeleton({
  className,
  showIcon = false,
  ...props
}: React.ComponentProps<"div"> & {
...
function CustomSidebarMenuSubItem({
  className,
  ...props
}: React.ComponentProps<"li">) {
  return (
...
export {
  ...
  CustomSidebarMenuAction,
  CustomSidebarMenuBadge,
